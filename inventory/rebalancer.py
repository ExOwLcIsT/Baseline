...
class RebalancePlanner:
    ...
    def plan(self, asset: str) -> list[TransferPlan]:
        ...
        assetMinAmount: Decimal = MIN_OPERATING_BALANCE.get("asset", Decimal(0))

        surplus = [pair for pair in adjustments if (pair[1] - assetMinAmount > 0)]
        ...
        for surPair in surplus:
            for defPair in deficit:
                if surPair[1] <= 0 or defPair[1] >= 0:
                    return
                transferAmount = Decimal.min(surPair[1], abs(defPair[1]))
                timeMin = (
                    TRANSFER_FEES[asset].estimatedTimeMin
                    if TRANSFER_FEES.get("asset") is not None
                    else 0
                )
                estimatedFee = (
    ...
    def plan_all(self) -> dict[str, list[TransferPlan]]:
        ...
        """
        assets: {str} = {
            [b.asset for b in self.tracker.balances]
