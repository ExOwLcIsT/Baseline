...
async def load_token(addr: str, client: ChainClient) -> Token:
    ...
    symbol, decimals = await asyncio.to_thread(lambda: (contract.functions.symbol().call(), contract.functions.decimals().call()))
...
async def main():
    ...
    if len(sys.argv) < 2:
        print("Usage: py -m scripts.price_impact_analyzer.py <pair_address> --token-in=USDC --sizes=1000,10000,100000")
        sys.exit(1)
    ...
    abi = [
        {"constant": True, "inputs": [], "name": "token0", "outputs": [
            {"name": "", "type": "address"}], "type": "function"},
        {"constant": True, "inputs": [], "name": "token1", "outputs": [
            {"name": "", "type": "address"}], "type": "function"},
        {"constant": True, "inputs": [], "name": "getReserves", "outputs": [{"name": "_reserve0", "type": "uint112"}, {
            "name": "_reserve1", "type": "uint112"}, {"name": "_blockTimestampLast", "type": "uint32"}], "type": "function"}
    ]

    contract = client.w3.eth.contract(
        address=Web3.to_checksum_address(pair_addr_str), abi=abi)
    token0_addr, token1_addr, reserves = await asyncio.to_thread(lambda: (
    ...
    token_in = token0 if token_in_symbol == token0.name else token1 if token_in_symbol == token1.name else None
    if not token_in:
        raise ValueError(
    ...
    # Table
    print("-" * 66)
    print(f"│ {'In'.rjust(10)} │ {'Out'.rjust(22)} │ {'Exec Price'.rjust(12)} │ {'Impact %'.rjust(9)} │")
    print("-" * 66)
    ...
    for r in rows:
        print(f"│ {format_number(r['amount_in']).rjust(10)} │ {format_number(r['amount_out'] / token_out.decimals).rjust(22)} │ {format_number(r['execution_price'], 6).rjust(12)} │ {format_number(r['price_impact_pct'], 4).rjust(9)} │")
