...
class ArbChecker:
    ...
    async def check(self, pair: str, size: float) -> dict:
        ...
        """
        POOL_ADDRESS = Address.from_string(
        ...
        eth_token = uni_pair.token0 if uni_pair.token0.name == "WETH" else uni_pair.token1
        usdt_token = uni_pair.token0 if uni_pair.token0.name == "USDT" else uni_pair.token1
        ...
        dex_quote = self.pricing_engine.get_quote(
            eth_token, usdt_token, size_int, 0)

        dex_gas = (
            Decimal(str(uni_pair.get_amount_out(
                dex_quote.gas_estimate * 10**9, eth_token)))
            / Decimal(str(usdt_token.decimals))
        )
        ...
        gap1 = dex_sell_price_usd - \
            buy_walked["avg_price"]   # buy CEX, sell DEX
        gap2 = sell_walked["avg_price"] - \
            dex_buy_price_usd   # buy DEX, sell CEX

        if gap1 > gap2:
            gap_bps = gap1 / buy_walked["avg_price"] * Decimal("10000")
            cex_slippage_bps = buy_walked["slippage_bps"]
            dex_price_impact = Decimal(str(
                uni_pair.get_price_impact(size_int, eth_token) / 100
            ))
            gas_bps = dex_gas / dex_sell_price_usd * Decimal("10000")
            inventory_ok = (
                self.inventory_tracker.get_available(
                    Venue.BINANCE, "USDT") >= buy_walked["total_cost"]
                and self.inventory_tracker.get_available(Venue.WALLET, "ETH") >= Decimal(str(size))
            )
        else:
            gap_bps = gap2 / dex_buy_price_usd * Decimal("10000")
            cex_slippage_bps = sell_walked["slippage_bps"]
            dex_price_impact = Decimal(str(
                uni_pair.get_price_impact(dex_buy_price_raw, usdt_token) / 100
            ))
            gas_bps = dex_gas / dex_buy_price_usd * Decimal("10000")
            inventory_ok = (
                self.inventory_tracker.get_available(
                    Venue.WALLET,  "USDT") >= Decimal(str(dex_buy_price_raw))
                and self.inventory_tracker.get_available(Venue.BINANCE, "ETH") >= Decimal(str(size))
            )

        CEX_FEE_BPS = Decimal("10")
        DEX_FEE_BPS = Decimal("30")
        estimated_costs_bps = dex_price_impact + \
            cex_slippage_bps + CEX_FEE_BPS + DEX_FEE_BPS + gas_bps
        ...
        dex_price = dex_sell_price_usd if direction == "buy_cex_sell_dex" else dex_buy_price_usd
...
async def main():
    ...
    if checked["direction"] == "buy_cex_sell_dex":
        ...
        print(
            f"   Uniswap V2:    ${checked['dex_price']:.2f} (buy {size} ETH)")
    ...
    print(
        f"  Gas:               ${d['gas_cost_usd']:.4f} ({d['gas_bps']:.2f} bps)")
    print("─" * 65)
    ...
    if checked["direction"] == "buy_dex_sell_cex":
        wallet_usdt = arb_checker.inventory_tracker.get_available(
            Venue.WALLET,  "USDT")
        binance_eth = arb_checker.inventory_tracker.get_available(
            Venue.BINANCE, "ETH")
        ...
        binance_usdt = arb_checker.inventory_tracker.get_available(
            Venue.BINANCE, "USDT")
        wallet_eth = arb_checker.inventory_tracker.get_available(
            Venue.WALLET,  "ETH")
