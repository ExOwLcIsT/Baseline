...
class RiskManager:
    ...
    def check_pre_trade(self, signal: Signal) -> tuple[bool, str | None]:
        ...
        if trade_value > self.risk_limits.max_trade_usd:
            return False, f"Trade value ${trade_value:.2f} exceeds max ${self.risk_limits.max_trade_usd}"
        ...
        if trade_value > self.current_capital * self.risk_limits.max_trade_pct:
            ...
            return False, f"Trade is {pct:.1f}% of capital, max {self.risk_limits.max_trade_pct * 100:.0f}%"
        ...
        if current_exposure + trade_value > self.risk_limits.max_position_per_token:
            return False, f"Position in {base} would exceed max ${self.risk_limits.max_position_per_token}"

        # 4. Max open positions
        if len(self.open_positions) >= self.risk_limits.max_open_positions:
            return False, f"Max open positions ({self.risk_limits.max_open_positions}) reached"

        # 5. Max loss per trade (expected)
        if abs(min(signal.expected_net_pnl, 0)) > self.risk_limits.max_loss_per_trade:
            return False, f"Expected loss exceeds max per-trade loss ${self.risk_limits.max_loss_per_trade}"
        ...
        if drawdown >= self.risk_limits.max_drawdown_pct:
            return False, f"Drawdown {drawdown*100:.1f}% exceeds max {self.risk_limits.max_drawdown_pct*100:.0f}%"

        # 8. Max trades per hour
        one_hour_ago = time.time() - 3600
        recent_trades = [
            t for t in self.trade_history if t.timestamp > one_hour_ago]
        if len(recent_trades) >= self.risk_limits.max_trades_per_hour:
            return False, f"Max trades per hour ({self.risk_limits.max_trades_per_hour}) reached"

        # 9. Consecutive loss limit
        if self.consecutive_losses >= self.risk_limits.consecutive_loss_limit:
            return False, f"Consecutive loss limit ({self.risk_limits.consecutive_loss_limit}) reached"
