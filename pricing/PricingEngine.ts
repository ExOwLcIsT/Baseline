...
export default class PricingEngine {
  ...
  async swap(size: Decimal, tokenIn: Token, tokenOut: Token) {
    ...
    const tx = (
      await (
        await new TransactionBuilder(this.client, wallet)
          .to(Address.fromString(process.env.UNISWAP_V2_ROUTER_ADDRESS!))
          .value(
            TokenAmount.fromHuman(
              tokenIn.address.checksum ===
