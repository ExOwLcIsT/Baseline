...
`);
  ...
  const tokenIn =
    tokenInSymbol.toUpperCase() === token0.name
      ? token0
  ...
  if (!tokenIn)
    throw new Error(
      `Token ${tokenInSymbol} not found in pair (${token0.name}/${token1.name})`,
    );
  ...
  console.log(
    `Reserves: ${format(Number(pair.reserve0))} ${token0.name} / ${format(
      Number(pair.reserve1),
    )} ${token1.name}`,
  );

  console.log(
    `Spot Price: ${format(spot, 6)} ${tokenIn.name}/${tokenOut.name}\n`,
  );
  ...
  console.log(
    `│ ${"In".padStart(10)} │ ${"Out".padStart(22)} │ ${"Exec Price".padStart(
      12,
    )} │ ${"Impact %".padStart(9)} │`,
  );
  console.log("-".repeat(66));
  ...
  for (const r of rows) {
    console.log(
      `│ ${format(r.amountIn).padStart(10)} │ ${format(r.amountOut).padStart(
        22,
      )} │ ${format(r.executionPrice, 6).padStart(
        12,
      )} │ ${format(r.priceImpactPct, 4).padStart(9)} │`,
    );
  }
