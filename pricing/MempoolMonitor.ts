import { WebSocketProvider, TransactionResponse, Interface } from "ethers";
...
type Callback = (swap: ParsedSwap) => void;
...
export default class MempoolMonitor {
  ...
  static SWAP_SELECTORS: Record<string, [string, string]> = {
  ...
  async start() {
    ...
    this.provider.on("pending", async (hash: string) => {
      try {
        ...
        if (parsed) this.callback(parsed);
      } catch {
  ...
  parseTransaction(tx: TransactionResponse): ParsedSwap | undefined {
    ...
    const amountIn: bigint = params.amountIn ?? 0n;
    const minOut: bigint = params.amountOutMin ?? 0n;
  ...
  decodeSwapParams(selector: string, data: string): any {
    try {
      switch (selector) {
        case "0x38ed1739": {
          const decoded = V2_ABI.decodeFunctionData(
  ...
  private calcSlippage(amountIn: bigint, minOut: bigint): number {
    if (amountIn === 0n || minOut === 0n) return 0;
